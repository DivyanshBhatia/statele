...
2. **Capital** — type the capital city, with a type-ahead list (3 attempts).
3. **Pinpoint the capital** — pick the capital's location from points on the map.
4. **Locate on the map** — tap the state's position on the map of India; tiny states and UTs are marked with a dot so they're easy to tap.
...
8. **Spot the landmark** — pick the state's landmark from four photos (no names ever; if a photo can't be found, that option is swapped for another landmark that has one).
...
11. **Languages** — type the two most widely spoken languages, with a type-ahead list.
...
Earn a **badge** for every round you clear. You can step back to an earlier round with **"← Previous round"**, and each round has a **"See answer & skip to next round"** button if you're stuck, and a **"Give up & see results"** link to
...
- **Lakshadweep** is a scattered archipelago, so it renders as a cluster of tiny
  islands (its real geography) rather than a single recognizable shape — lean on the
  distance and direction clues there.
